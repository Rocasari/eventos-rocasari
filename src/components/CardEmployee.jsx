...
export default function CardEmployee({ empleado }) {  
  ...
  const handleDelete = async () => {  
    const confirmDelete = confirm(  
      `¿Estás seguro de que deseas eliminar a este empleado "${empleado.nombres} ${empleado.apellidos}"?`  
    );  
    if (!confirmDelete) return;  

    try {  
      const response = await fetch(  
        `http://localhost:8002/api/empleados/${empleado.idEmpleado}`, // Endpoint para eliminar empleado  
        {  
          method: "DELETE",  
          headers: {  
